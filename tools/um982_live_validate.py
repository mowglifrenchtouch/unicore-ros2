...
class TransportParser:
    ...
    def _try_extract_binary(self) -> Optional[object]:
        ...
        frame_size = 24 + payload_length + 4
...
class LiveValidator:
    ...
    def _clear_output_schedule(self) -> None:
        ...
        if result.status in {"unsupported", "no_response"}:
...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Capture and validate a live UM982/Unicore serial stream without ROS 2."
    )
